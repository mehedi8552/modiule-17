...
exports.createOder = async (req, res) => {
  try {
    ...
    res.status(404 ).json({ message: "got error", error: err.message });
  }
};
exports.OderListUpdate = async (req, res) => {
  ...
  try {
    ...
    if (result.nModified > 0) {
      ...
      res
        .status(200).json({ message: "Order update successful", data: result });
    }
  } catch (err) {
    res.status(404).json({ message: "Got an error", error: err.message });
  }
};
